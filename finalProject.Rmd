...
scatter_data <- final_data %>% st_drop_geometry() %>%
  select(walk_score, transit_distance_km) %>% na.omit()

r2_trn <- round(cor(scatter_data$walk_score, scatter_data$transit_distance_km)^2, 3)

ggplot(scatter_data, aes(x = transit_distance_km, y = walk_score)) +
  geom_point(alpha = 0.35, color = "#e6550d") +
  geom_smooth(method = "lm", se = TRUE, color = "#a63603") +
  scale_y_continuous(limits = c(0, 20)) +
  labs(
    title = "Distance to Nearest Transit Stop vs Walkability",
    subtitle = paste0("R² = ", r2_trn, " — strongest single predictor in the dataset"),
    x = "Distance to Nearest Transit Stop (km)",
    y = "Walkability Score",
    caption = "Each point = one census tract (Greater Chicago metro area, n ≈ 2,009)"
  ) +
